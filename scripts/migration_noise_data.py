...
def open_raw_data():
    with open('../fixtures/neighborhood_new.json','r') as fn:
        data = json.load(fn)
        return data
...
def calculate_crime_rate(data):
    ...
    for item in data:
        if 'noise_count' in  item['fields']:
            ...
            if noise_rate > hightest_crime_rate:
                ...
                if name in n['fields']['name'] and 'noise_count' in n['fields']:
                    ...
                    for p in range(len(rate_standard)):
                        if noise_rate <= rate_standard[p]:
                            item['properties']['noise'] = p+1
                            break
                    break
                else:
                    item['properties']['noise'] = 0
    print(geojson)
    return geojson
...
if __name__ == '__main__':
    ...
    with open('../static/data/tabulation_new2.geojson', 'w') as f:
